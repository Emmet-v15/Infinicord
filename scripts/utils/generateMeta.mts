...
const releases = await fetch("https://api.github.com/repos/Equicord/Infinibop/releases", {
    headers: {
...
const metaInfo = await (async () => {
    for (const release of releases) {
        const metaAsset = release.assets.find((a: any) => a.name === "org.equicord.infinibop.metainfo.xml");
...
await fs.writeFile("./dist/org.equicord.infinibop.metainfo.xml", output, "utf-8");
console.log("Updated meta information written to ./dist/org.equicord.infinibop.metainfo.xml");